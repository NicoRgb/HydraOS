...
if [[ $HYDRAOS_BOOT_SYSTEM == 'UEFI' ]]; then
    ...
    qemu-system-x86_64 -drive file=../hydraos.img,format=raw -debugcon file:/dev/stdout -no-shutdown -no-reboot -cpu qemu64 -device virtio-gpu-pci -device virtio-rng-pci -display sdl -d guest_errors
fi
